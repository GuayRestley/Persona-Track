...
function addAccount($conn) {
    ...
    // Insert new account
    $sql = "INSERT INTO accounts (username, password_hash, role, status) 
...
function updateAccount($conn) {
    ...
    if ($conn->query($sql)) {
        // Log activity
        logActivity($conn, $_SESSION['account_id'] ?? 1, 'Update', "Updated account: $username");
...
function deleteAccount($conn) {
    ...
    if ($conn->query($sql)) {
        // Log activity
        logActivity($conn, $_SESSION['account_id'] ?? 1, 'Delete', "Deleted account: $username");
...
function logActivity($conn, $account_id, $action_type, $description) {
    ...
    $sql = "INSERT INTO activity_logs (account_id, action_type, action_description, timestamp) 
