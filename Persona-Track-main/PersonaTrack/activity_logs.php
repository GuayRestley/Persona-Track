...
// Get the dashboard URL for use in the page
...
function getLogStats($conn) {
    ...
    // This week's logs
    $result = $conn->query("SELECT COUNT(*) as count FROM activity_logs WHERE YEARWEEK(created_at) = YEARWEEK(NOW())");
    if ($result) {
        $stats['this_week'] = $result->fetch_assoc()['count'];
    }
    
    // This month's logs
    $result = $conn->query("SELECT COUNT(*) as count FROM activity_logs WHERE MONTH(created_at) = MONTH(NOW()) AND YEAR(created_at) = YEAR(NOW())");
    if ($result) {
...
function fetchLogs($conn) {
    ...
    if ($result) {
        ...
        echo json_encode(['success' => false, 'message' => 'Error fetching logs: ' . $conn->error]);
    }
}
...
function deleteLog($conn) {
    ...
    if ($conn->query($sql)) {
        ...
        echo json_encode(['success' => false, 'message' => 'Error deleting log: ' . $conn->error]);
    }
}
...
function clearLogs($conn) {
    ...
    if (!empty($date_from)) {
        $date_from = $conn->real_escape_string($date_from);
        $sql = "DELETE FROM activity_logs WHERE DATE(created_at) < '$date_from'";
    } else {
    ...
    if ($conn->query($sql)) {
        ...
        echo json_encode(['success' => true, 'message' => "Successfully cleared $affected_rows old logs"]);
    } else {
        echo json_encode(['success' => false, 'message' => 'Error clearing logs: ' . $conn->error]);
    }
}
?>
